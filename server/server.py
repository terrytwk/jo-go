...
def request_handler(request):
    ...
    if request["method"] == "GET":
        ...
        if 'item' in request['form']: 
            ...
            encoded_id = c.execute('''SELECT id from swipe_history ORDER BY time DESC LIMIT 1''').fetchone()[0]
            conn = sqlite3.connect(JOGO_DB_LOCATION)
            ...
            if 'count' in request['form']:
                ...
                return json.dumps({"id": encoded_id, "item": item, "item_count": int(count)})
            ...
            if 'action' in request['form']:
                ...
                return json.dumps({"id": encoded_id, "item": item, "action": action})
